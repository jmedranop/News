...
*/
	...
	function parse()
	{
		...
		if ($xml->channel)
		{
			...
			$this->channel_data['imagen_url'] = $xml->channel->image->url;
			$data['titulo_head'] = (string)$xml->channel->title;
			$data['descripcion_head'] = (string)$xml->channel->description;
			//$data['img_head'] = (string)$xml->channel->image->url;

			// Build the item array
			foreach ($xml->channel->item as $item)
			{
				$data['id'] = (string)$item->id;
				$data['titulo'] = (string)$item->title;
				$data['descripcion'] = (string)$item->description;
				$data['fecha'] = (string)$item->pubDate;
				$data['url'] = (string)$item->link;
				$dc = $item->children('http://purl.org/dc/elements/1.1/');
				$data['autor'] = (string)$dc->creator;
				$data['categoria'] = (string)$item->category;
				$data['contenido'] = (string)$item->content->img;
				if(isset($namespaces['media']))
				{
					$data['img_head'] = trim((string)$item->children($namespaces['media'])->thumbnail->attributes()->url);
				}
				else
				{
					$data['img_head'] = extraerSRC((string)$item->content);
				}
		...
		else
		{
			...
			$data['titulo_head'] = (string)$xml->title;
			$data['descripcion_head'] = (string)$xml->description;
			//$data['img_head'] = (string)$xml->image->url;
			// Build the item array
			...
			foreach ($xml->entry as $item)
			{
				...
				$data['id'] = (string)$item->id;
				$data['titulo'] = (string)$item->title;
				$data['descripcion'] = (string)$item->content;
				$data['fecha'] = (string)$item->published;
				$data['url'] = (string)$item->link['href'];
				$dc = $item->children('http://purl.org/dc/elements/1.1/');
				$data['autor'] = (string)$dc->creator;
				$data['categoria'] = (string)$item->category;
				$data['img_head'] = extraerSRC((string)$item->content);
				echo "images---->".$data['img_head'];
				if(empty($data['img_head']))
					$data['img_head'] = trim((string)$item->children($namespaces['media'])->thumbnail->attributes()->url);
				if ($this->callback)
				{
					$data = call_user_func($this->callback, $data, $item);
